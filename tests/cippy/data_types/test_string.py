...
@pytest.mark.parametrize(
    "typ, val, bites",
    string_tests,
    ids=[f"string_{i}" for i in range(len(string_tests))],  # fix error with env var size limits on windows
)
...
@pytest.mark.parametrize(
    "typ, val, bites",
    str_maxes,
    ids=[f"string_{i}" for i in range(len(str_maxes))],  # fix error with env var size limits on windows
)
...
def test_stringn_default():
    ...
    _str = STRINGN(val)
    assert _str == val
    assert _str.encoding == "utf-8"
    assert _str == STRINGN.decode(bytes(_str))


stringn_tests = [
    # string val, [char size][char count][string], encoding
    ("hello there", b"\x01\x00\x0b\x00hello there", STRINGN.Encoding.utf_8),
    (
        "general kenobi",
        b"\x02\x00\x0e\x00g\x00e\x00n\x00e\x00r\x00a\x00l\x00 \x00k\x00e\x00n\x00o\x00b\x00i\x00",
...
def test_stringn(val, bites, encoding):
    ...
    assert _str.encoding == STRINGN._encodings[encoding]  # instance var set for encoding
    assert STRINGN.encoding == "utf-8"  # don't override class var
...
def test_stringi(
    strings: list[tuple[str, type[STRING | STRING2 | STRINGN | SHORT_STRING], STRINGI.Language, STRINGI.CharSet]],
    bites: bytes,
):
